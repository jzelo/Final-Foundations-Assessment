...
// The only thing i was not able to get was the import function to work, i continually got an error when trying to display
//it on the screen and I tried making an JSON page, i tried using type: module, and i also asked about it but
//didnt get an answer so i had to put the code below the array
const form = document.querySelector("form");
const submitBtn = document.getElementById("submitBtn");
const container = document.getElementById("booksContainer");

//making a submit button that will allow the user to create a book and submit the information in the correct way
submitBtn.addEventListener("click", function(event) {
  ...
  const newBook = { title, author, subject, language };
  bookData.push(newBook);
  const newBookEl = createBookElement(newBook);
  container.appendChild(newBookEl);
});

//maping the books from the array so that they can be viewed correctly and not just as a giant array
const bookElements = bookData.map(createBookElement);
...
function createBookElement(book) {
  ...
  // Add event listener for form submit for the comments
  const form = bookEl.querySelector(".comment-form");
  form.addEventListener("submit", event => {
    event.preventDefault();
    const textarea = bookEl.querySelector(".comment-text");
    const comment = textarea.value;
    textarea.value = '';

    //make the comments container a child of the form so that it will attach corrently
    const commentsContainer = document.createElement("div");
    commentsContainer.classList.add("comments");
    commentsContainer.appendChild(form);
    bookEl.appendChild(commentsContainer);

    // Get the comments container
    if (commentsContainer.contains(form)) {
        // Create a new element for the comment
        const commentEl = document.createElement("div");
        commentEl.innerHTML = comment;
        // Insert the comment element before the form element
...
// making the search function work by searching for books with different criteria and making it so that the font doesnt matter
const searchInput = document.getElementById("searchInput");
searchInput.addEventListener("keyup", function() {
  const searchQuery = this.value.toLowerCase();
  const filteredBooks = bookData.filter(book =>
    (typeof book.title === 'string' && book.title.toLowerCase().includes(searchQuery)) ||
    (typeof book.author === 'string' && book.author.toLowerCase().includes(searchQuery)) ||
    (typeof book.subject === 'string' && book.subject.toLowerCase().includes(searchQuery)) ||
    (typeof book.language === 'string' && book.language.toLowerCase().includes(searchQuery))
);

  container.innerHTML = "";
  const filteredBookElements = filteredBooks.map(createBookElement);
  filteredBookElements.forEach(bookEl => {
    container.appendChild(bookEl);
  });
});
...
bookElements.forEach(bookEl => container.appendChild(bookEl));




//All of this is different code that i tried to write and work with that i couldnt get to work correctly for the
//past week, i just decided to leave it to show that i tried several different things before i got something to finally
//work together. And the code above should now all work together!


// // Create a Book constructor function
// function Book(title, author, subjects, language) {
//   this.title = title;
//   this.author = author;
...
// // Create a Bookshelf constructor function
// function Bookshelf() {
//   this.books = [];
//   this.favoriteBook = [];
// }

// // Add a 'addBook' method to the Bookshelf prototype
...
// // Iterate over the book data and create Book objects
// bookData.forEach(function(book) {
//   let newBook = new Book(book.title, book.author, book.subjects, book.language);
...
//   const filteredBooks = bookData.filter(book => book.title.includes(searchQuery) || book.author.includes(searchQuery) || book.subjects.includes(searchQuery));
...
//       <p>${book.subjects}</p>
//     `;
//     console.log("Book element: ", bookElement); //log the book element
//     return bookElement;
//   });
//   const bookshelf = document.getElementById('search-results');
//   bookshelf.innerHTML = "";
//   bookElements.forEach(bookElement => bookshelf.appendChild(bookElement));
// });



// // Add a 'render' method to the Bookshelf prototype
// Bookshelf.prototype.render = function(books) {
//   if (!books) {
...
//           <p>Language: ${book.language}</p>
//       `;
// //Add a comment section to the books
//       let commentElement = document.createElement('div');
//       commentElement.innerHTML = `<h4>Comments:</h4>`;
...
//           });
//       }
// // Making the criteria for the comments and adding character limits
//       let inputComment = document
//       .createElement('input');
//       inputComment.setAttribute('placeholder','leave a comment');
//       inputComment.setAttribute('type','text');
//       inputComment.setAttribute('id', 'input-'+ book.title);
//       let addCommentButton = document.createElement('button');
//       addCommentButton.innerHTML = "Add comment";
//       addCommentButton.addEventListener('click', function(){
//           let input = document.getElementById('input-'+ book.title).value;
//           if (input && input.length <= 280){
//               if(!book.comments) book.comments = []
//               book.comments.push(input);
//               myBookshelf.render();
//           }
//           else if (input && input.length > 280) alert("Your comment exceed 280 characters, please try again with a shorter comment.")
//       });
//       bookElement.appendChild(commentElement);
//       bookElement.appendChild(inputComment);
//       bookElement.appendChild(addCommentButton);

// // Add a favorites button to the bottom of each book on the bookshelf
//       let favoriteButton = document.createElement('button');
//       favoriteButton.innerHTML = "Add to Favorites";
//       favoriteButton.addEventListener('click', function(){
//           myBookshelf.favoriteBook.push(book);
//       });
//       bookElement.appendChild(favoriteButton);

//       // Append the book element to the bookshelf element
//       document.getElementById('search-results').appendChild(bookElement);
//   });
// }

// // Add a few books to the bookshelf
// myBookshelf.addBook(new Book('The Great Gatsby', 'F. Scott Fitzgerald', 'Novel', 'English'));
// myBookshelf.addBook(new Book('The Catcher in the Rye', 'J.D. Salinger', 'Novel', 'English'));
// myBookshelf.addBook(new Book('The Alchemist', 'Paulo Coelho', 'Novel', 'Portuguese'));
...
//     });
//     bookElement.appendChild(commentSection);
//     // Create input and button for new comments
//     const inputComment = document.createElement('input');
//     inputComment.setAttribute('placeholder', 'Leave a comment');
//     inputComment.setAttribute('type', 'text');
//     inputComment.setAttribute('id', `input-${this.title}`);
//     const addCommentButton = document.createElement('button');
// addCommentButton.innerHTML = "Add Comment";
// addCommentButton.addEventListener('click', () => {
//     this.comments.push(inputComment.value);
//     inputComment.value = "";
//     this.render();
// });
//     bookElement.appendChild(inputComment);
...
//   const searchQuery = document.querySelector('.search-query').value;
//   const filteredBooks = bookData.filter(book => book.title.includes(searchQuery) || book.author.includes(searchQuery) || book.subject.includes(searchQuery));
//   const bookElements = filteredBooks.map(book => {
//     const bookElement = document.createElement('li');
//     bookElement.innerHTML = `
//       <h3>${book.title}</h3>
...
//   // Call the render method on the bookshelf instance
//   const bookshelfElement = bookshelf.render();
//   document.getElementById('bookshelf').appendChild(bookshelfElement);
...
// // Add a 'search' method to the Bookshelf prototype
// Bookshelf.prototype.search = function(query) {
//   const searchResults = Bookshelf.search(query);

//   // Convert the query to lowercase
//   query = query.toLowerCase();
  
//   // Use the filter method to return the books that match the query
//   let results = this.books.filter(book => book.title.toLowerCase().includes(query) || book.author.toLowerCase().includes(query));
  
//   results.forEach(book => {
//     if (book.title.toLowerCase() === query || book.author.toLowerCase() === query) {
//       results.unshift(results.splice(results.indexOf(book), 1)[0]);
//     }
//   });
//   this.books = results;
//   this.render();
...
//   const searchResults = myBookshelf.search(query);
...
//     searchResultsContainer.appendChild(newBook);
//   });
// });
...
// // Import the data for the bookshelf from book-data.js
// // import bookData from './book-data';
// // This did not work, every time i put "type="module"" in the script portion i got a CORS error that would not allow me to get past
// // I know that i shouldn't just put the entire array in with the rest of the JS file, but i didn't know how else to get around it
// // The ways to get past it was to use something called a JSONP and something else called a middlewear, neither of which seemed safe
...
// // Add a 'search' method to the Bookshelf prototype
// Bookshelf.prototype.search = function(query) {
//   let results = this.books.filter(function(book) {
//       return book.title.toLowerCase().includes(query) || book.author.toLowerCase().includes(query);
//   });
// // Adding a loop that runs through the results of the search and splices the array to make the searched book at the top
//   for(let i=0; i < results.length; i++){
//       if(results[i].title.toLowerCase() === query.toLowerCase() || results[i].author.toLowerCase() === query.toLowerCase()){
//           let matchedBook = results.splice(i,1);
//           results.unshift(matchedBook[0]);
//           break;
//       }
//   }
//   return results;
...
//     favoriteButton.addEventListener("click", function(){
//         myBookshelf.favoriteBooks.push(book);
//     });
//     bookElement.appendChild(favoriteButton);
//     document.getElementById('bookshelf').appendChild(bookElement);
//   });
// }
...
// // When the 'Add Book' button is clicked, add a new book to the bookshelf and re-render
// document.getElementById('add-book').addEventListener('click', function() {
//   let title = document.getElementById('title').value;
//   let author = document.getElementById('author').value;
//   let subjects = document.getElementById('subjects').value;
//   let language = document.getElementById('language').value;
//   myBookshelf.addBook(new Book(title, author, subjects, language));
//   myBookshelf.render();
// });

// // Listen to the input event on search-query element 
// document.getElementById("search-query").addEventListener("input", function(){
//   let searchResults = myBookshelf.search(this.value);
//   myBookshelf.render(searchResults);
// });

// // When the search book button is clicked, search for the book and re-render the bookshelf
// document.getElementById('search-book').addEventListener('click', function(){
//   let searchQuery = document.getElementById('search-query').value;
//   let searchResults = myBookshelf.search(searchQuery.toLowerCase());
//   myBookshelf.render(searchResults);
// });
